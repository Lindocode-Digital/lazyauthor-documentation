...
3. [Interactive Story Editor Walkthrough](#3-interactive-story-editor--walkthrough)
   ...
   - [Writing a variable-driven story](#34-writing-a-variable-driven-story)
   - [Gating choices with conditions](#35-gating-choices-with-conditions)
   - [Collecting reader input](#36-collecting-reader-input)
   - [Ending a branch](#37-ending-a-branch)
   - [Attaching media](#38-attaching-media)
   - [Filling in story details](#39-filling-in-story-details)
   - [Styling your story](#310-styling-your-story)
   - [Previewing a scene](#311-previewing-a-scene)
   - [Exporting your story](#312-exporting-your-story)
   - [Importing an existing story](#313-importing-an-existing-story)
4. [Book Editor - Walkthrough](#4-book-editor--walkthrough)
   - [Your first chapter](#41-your-first-chapter)
   - [Adding and reordering chapters](#42-adding-and-reordering-chapters)
   - [Writing chapter content](#43-writing-chapter-content)
   - [Adding images to a chapter](#44-adding-images-to-a-chapter)
   - [Filling in book details](#45-filling-in-book-details)
   - [Styling your book](#46-styling-your-book)
   - [Previewing a chapter](#47-previewing-a-chapter)
   - [Exporting your book](#48-exporting-your-book)
   - [Importing an existing file](#49-importing-an-existing-file)
5. [Common workflows](#5-common-workflows)
   - [Name the player and use it throughout](#51-name-the-player-and-use-it-throughout)
   - [Track a score and show it on an ending screen](#52-track-a-score-and-show-it-on-an-ending-screen)
   - [Lock a choice behind an item](#53-lock-a-choice-behind-an-item)
   - [Build a timed puzzle](#54-build-a-timed-puzzle)
   - [Write a book with a prologue](#55-write-a-book-with-a-prologue)
...
1. Go to [lazyauthor.lindocode.com](https://lazyauthor.lindocode.com).
...
Inside any scene's narrative body, type `{{variableName}}` wherever you want the value to appear. At read time this is replaced with the current value.

Example:

```
You step forward. Your courage is at {{courage}} — 
{{courage >= 5}}enough to face whatever lies ahead.
```

For inline arithmetic (display only, does not change the variable):

```
If you succeed, your score will be {{score + 10}}.
```
...
A **Player Input** scene pauses the story and asks the reader to type something. The response is stored in a variable and can be used anywhere afterwards via `{{variableName}}`.
...
- `Text` — any string (default)
- `Number` — numeric keyboard on mobile, rejects non-numeric input
- `Password` — characters are hidden (useful for code words)
...
After uploading images (see [Adding images](#44-adding-images-to-a-chapter)), type `[image]` where you want the first image to appear, `[image2]` for the second, and so on.
...
3. In *The Road Ahead* and any later scene, write `{{playerName}}` wherever you want to address the reader.

   ```
   Welcome to the crossroads, {{playerName}}. 
...
3. Create an **Ending** scene. In the narrative:
   ...
   {{score >= 50}}You faced every challenge with courage. A true hero.
   ```

   > The `{{score >= 50}}` expression displays the calculated value — it does not hide content. To conditionally show entire sentences, put the condition on a choice that leads to one of two different ending scenes.
...
Draw your scene tree on paper or in a simple diagram tool before opening the editor. A story with 5 choices per scene and 3 levels deep has 156 possible scenes. Know roughly how big you want it before you start.

**Name scenes clearly**
Scene names are shown in every choice dropdown. Vague names like *Scene 12* or *New Scene* make it impossible to navigate a large story. Use names like *Forest Entrance*, *East Path — Day 2*, *Ending — Coward's Way* so you always know what you are looking at.

**Use one ending per branch, not one global ending**
Avoid funnelling all paths into a single ending scene. Each distinct outcome should have its own scene — this gives you room to write a tailored conclusion for each path.
...
**Test with Preview before exporting**
Use the **Preview** toggle on key scenes to check that your narrative reads correctly, variable expressions look right, and conditions hide the right choices. Export only when you are happy with the result.

**Export early and often**
Do not wait until the story is finished to export. Export a draft, open it in Lazy Reader, and experience it as a reader would. You will notice problems (confusing branching, missing scenes, awkward prose) that are invisible in the editor.

**Back up before importing**
Importing replaces your current work. If you want to keep your current story, export it to an EPUB first. That export is your backup.
...
Use a 1600 × 2400 px image (2:3 ratio) for covers. Wider or square images will work but may appear letterboxed in some readers.
...
- Check you are using the right operator. `gt 3` means strictly greater than 3; use `gte 3` to include the value 3.

**The player name variable shows `{{playerName}}` in the preview instead of a value**
- The preview uses default values. If the default for `playerName` is blank, the template renders as the empty string.
- Set a test default (e.g. `Alex`) while developing, then clear it before final export.

**I imported a .docx and the chapters are not split correctly**
- LazyAuthor splits at Heading 1 (`H1`) tags. If your Word document uses regular bold text instead of Heading 1 for chapter titles, the import cannot detect the breaks.
- In Word, select each chapter title and apply the **Heading 1** paragraph style, then re-export the `.docx` and import again.

**The EPUB looks unstyled when I open it in Apple Books / Kobo**
- Check that the **Font family** in the Theme panel uses a web-safe font (e.g. `Georgia, serif` or `"Palatino Linotype", serif`) or a font the reader app bundles. Custom font files are not embedded in the EPUB.
- Some reader apps override font settings. This is normal — readers often have their own typography preferences set at the system level.
